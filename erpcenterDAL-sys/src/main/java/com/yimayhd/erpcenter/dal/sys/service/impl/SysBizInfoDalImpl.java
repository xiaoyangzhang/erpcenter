...
public class SysBizInfoDalImpl implements SysBizInfoDal {
    ...
    @Autowired
    private TransactionTemplate transactionTemplateSys;
    ...
    @Override
    public void addSysBizInfo(final SysBizInfo sysBizInfo, final SettleApply apply, final PlatformEmployeePo empPo, final String menuIds) {
    ...
    //@Transactional(propagation = Propagation.REQUIRED)
    @Override
    public void configSysBizInfoById(final SysBizInfo biz, final String menuIds) {
        Boolean transactionResult = transactionTemplateSys.execute(new TransactionCallback<Boolean>() {
            @Override
            public Boolean doInTransaction(TransactionStatus status) {
                try {
